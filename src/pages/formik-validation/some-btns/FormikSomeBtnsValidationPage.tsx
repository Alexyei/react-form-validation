...
const FormikSomeBtnsValidationPage = (props:any)=>{
    ...
    const formik = useFormik<FormValues & {submitter:number}>({
        initialValues: {
            username: '',
            email: '',
            password: '',
    ...
    return (
        <RoundedForm header={header} {...props}>
            ...
            <GradientBtn style={{width:"50%",marginRight: "0"}} onClick={() => {
                formik.setFieldValue('submitter', 0, false)
                formik.handleSubmit()
            }} disabled={!(formik.isValid && formik.dirty) || isLoading} isLoading={isLoading} type="button">{btnText}</GradientBtn>
            <GradientBtn style={{width:"50%",marginRight: "0"}} onClick={() => {
                formik.setFieldValue('submitter', 1, false)
                formik.handleSubmit()
            }} disabled={!(formik.isValid && formik.dirty) || isLoading} isLoading={isLoading} type="button">{btnText}</GradientBtn>
            </div>
        </RoundedForm>
...
export default FormikSomeBtnsValidationPage;